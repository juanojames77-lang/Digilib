...
// ✅ ADD THIS: Auto-check database on startup
async function checkDatabase() {
  try {
    ...
    if (!result.rows[0].exists) {
      console.log('⚠️ Database tables not found. Visit /setup to create them.');
    } else {
    ...
    console.log('💡 Make sure DATABASE_URL is correct and database is running');
  }
}
...
// ================= DATABASE SETUP PAGE =================
// ONE-TIME USE: https://your-app.onrender.com/setup
// PUT THIS RIGHT HERE - BEFORE ALL OTHER ROUTES!
app.get('/setup', async (req, res) => {
  try {
    ...
    const sql = `
      ...
      -- 10. CREATE TEST USER (password: user123)
      INSERT INTO users (username, password) 
      VALUES ('testuser', '$2a$10$N9qo8uLOickgx2ZMRZoMyeG3ZhokpQwA7c6.8ZPJk8hGt6p/Y.BCa')
      ON CONFLICT (username) DO NOTHING;
    `;
    ...
    res.send(`
      ...
      <head>
        <style>
          ...
          .btn { background: #2196F3; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; display: inline-block; margin-top: 20px; }
        </style>
      </head>
      <body>
        <div class="success">
